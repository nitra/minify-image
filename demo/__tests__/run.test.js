 ...
 * pngquant зі `strip: true` його викидає — фактичний розмір падає до базової PNG.
...
test('estimate-режим: реально проганяє imagemin для всіх форматів', async () => {
  const { exitCode, stdout } = await runCli([`--src=${filesDir}`], here)

  expect(exitCode).toBe(0)

  // Кожна фікстура має пройти imagemin (рядок з "original size:")
...
test('--write режим: проганяє imagemin для всіх файлів і наповнює cache', async () => {
  ...
  try {
    ...
    // На чистому cache кожен файл має пройти imagemin принаймні раз
    // (JPEG обробляється двома плагінами підряд — другий побачить cache-hit з першого)
    for (const name of expectedFiles) {
    ...
    // ready.png — навмисно товстий (PNG з 50 KB tEXt-баласту); має реально
    // стиснутися >15% завдяки pngquant strip:true + zopfli
...
test('--write: реально перезаписує файл коли економія >15%', async () => {
  ...
  try {
    ...
    // pngquant зі strip:true викидає tEXt-баласт; відсоток економії має перетнути поріг 15%
